...
export function Modal({ open, onClose, title, children, size = 'md' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] overflow-y-auto p-3 sm:p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 mx-auto flex min-h-full w-full items-start justify-center pb-4 pt-12 sm:items-center sm:py-4">
        <div className={`relative z-10 flex max-h-[calc(100dvh-4rem)] w-full max-w-[100vw] flex-col touch-pan-y overflow-y-auto overscroll-contain rounded-[1.5rem] border border-blue-300 bg-white text-slate-900 shadow-[0_26px_80px_rgba(11,31,68,0.3)] animate-scale-in sm:max-h-[min(90dvh,680px)] sm:w-auto sm:rounded-2xl ${maxW}`}>
          ...
          <div className="px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:px-6 sm:pb-6">
            {children}
          </div>
        </div>
      </div>
