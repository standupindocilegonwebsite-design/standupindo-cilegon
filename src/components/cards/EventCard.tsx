  ...
  return (
    <>
      <article
        ...
        className={`group cursor-pointer overflow-hidden ${isCompleted ? 'rounded-2xl border border-slate-300 bg-white shadow-[0_8px_20px_rgba(15,23,42,0.08)]' : 'card card-hover rounded-[22px] border border-slate-200 bg-white shadow-[0_10px_24px_rgba(15,23,42,0.04)]'}`}
        role="button"
        tabIndex={0}
      >
        <div className={isCompleted ? 'flex items-stretch' : ''}>
        <div className={`relative overflow-hidden bg-slate-100 ${isCompleted ? 'aspect-[16/10] w-[112px] shrink-0 sm:w-[150px]' : ''}`}>
          {event.poster && !posterFailed ? (
            <button onClick={() => setLightbox(true)} aria-label={`Lihat poster ${event.title}`} className="block h-full w-full">
              <img
                ...
                className={`h-full w-full transition-transform duration-500 ${isCompleted ? 'object-cover grayscale group-hover:scale-105' : 'aspect-[16/10] object-cover group-hover:scale-[1.03]'}`}
              />
            </button>
          ) : (
            <button onClick={() => router.navigate(`/event/${event.slug}`)} className={`flex w-full items-center justify-center bg-slate-100 text-sm font-semibold text-slate-400 ${isCompleted ? 'h-full' : 'aspect-[16/10]'}`}>
        ...
        <div className={isCompleted ? 'flex min-w-0 flex-1 flex-col p-3.5 sm:p-4' : 'p-4'}>
          ...
          <div className={`${isCompleted ? 'mt-auto pt-3' : 'mt-4 border-t border-slate-100 pt-3'}`}>
            <button onClick={() => router.navigate(`/event/${event.slug}`)} className={`btn-secondary w-full !justify-center !px-3 !py-2 text-[12px] font-semibold sm:!text-sm ${isCompleted ? '!rounded-lg' : ''}`}>
              Lihat Event
            </button>
          </div>
        </div>
